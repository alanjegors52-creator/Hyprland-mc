...
# ---------------- CONFIG ----------------
CONFIG_DIR = os.path.expanduser("~/.config/mc_menu")
...
def make_slot_button(app: dict | None, slot_size: int = 54) -> Gtk.Button:
    ...
    if app:
        ...
        btn.connect("clicked", lambda _w, cmd=app["cmd"]: launch(cmd))
...
class MinecraftMenu(Gtk.Window):
    def __init__(self):
        ...
        for idx, (sx, sy, ssize) in enumerate(slots):
            if idx >= len(apps):
                break
            btn = make_slot_button(apps[idx], ssize)
